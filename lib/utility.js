...
module.exports = {
     ...
     *  pointInBoundaries
     *
     *  Takes a point and checks if the point(lng, lat) is inside of boundaries
     *  defined be SouthWest and NordEast points
     *  SWlng <= Xlng <= NELng
     *  SWlat <= Xlat <= NELat
     **/
    pointInBoundaries: function(boundaries, x) {
        if( ((boundaries.SWLng <= x.lng) && (x.lng <= boundaries.NELng)) &&
            ((boundaries.SWLat <= x.lat) && (x.lat <= boundaries.NELat)) ) {
                return true;
        } else {
            return false;
        }
